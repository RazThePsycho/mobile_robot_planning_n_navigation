...
midposes = 10; 
idx_poses = 1:ceil(n/midposes)-1:n;
if idx_poses(end) ~= n
    idx_poses(end+1) = n;
end
...
% con giros
v_ack_turn = 1; % velocidad de avance
arange_ack_turn = [0.05:0.05:0.45 -0.05:-0.05:-0.45]; % % rango de angulos del volante
% arange_ack_turn = [0.1:0.1:0.7 -0.1:-0.1:-0.7];
% cost_turn =  2;
% cost_turnback = 15;
% cost_turn =  [       5    5   5    5   5    5   5    5];
cost_turn = 2*ones(1, length(arange_ack_turn));
cost_turnback = 3*ones(1, length(arange_ack_turn));
...
outfilename = 'rbcar_04.mprim';
fout = fopen(outfilename, 'w');
stderr = fopen('rbcar_err', 'w');
